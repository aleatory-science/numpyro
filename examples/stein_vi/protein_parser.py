...
def _convert_proteins(dicts):
    return {k: tuple(list(ll) for ll in zip(*v))
            for d in dicts for k, v in d.items()}


AMINO_ACIDS = ['A', 'R', 'N', 'D', 'C', 'Q', 'G', 'E', 'H', 'I', 'L',
               'K', 'M', 'F', 'P', 'S', 'T', 'W', 'Y', 'V']
...
class ProteinParser(TextParsers, whitespace=r'[ \t]*'):
    ...
    def parsef_jnp(cls, filename):
        ...
        for as_, ds, phis, psis in structured_data.values():
            inddata.append((list(map(AMINO_ACIDS.index, as_[1:-1])),
                            list(map(DSSP.index, ds[1:-1])),
                            phis[1:-1],
                            psis[1:-1]))
            lengths.append(len(as_[1:-1]))
        max_length = max(lengths)
        return *tuple(jnp.array([la + [0] * (max_length - len(la)) for la in ll])
                      for ll in zip(*inddata)), jnp.array(lengths)
